...
function ByteLengths(lengths)
{
  ...
  this._transform = function(chunk, encoding, callback)
  {
    ...
    cachedBytes = result.length % _length
    if(cachedBytes)
    {
      var length = result.length - cachedBytes
  ...
  this._flush = function(callback)
  {
    ...
    if(_length)
    {
      if(_length !== Math.Infinity) return callback('Waiting for data')

      this.truncate = cachedBytes
    }
